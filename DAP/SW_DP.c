		...
		uint32_t request, uint32_t *data)
{
	...
	PIN_DELAY_FAST();
	PIN_SWCLK_SET();
	PIN_DELAY_FAST(); /* Start Bit */
	bit = request >> 0;
	PIN_SWDIO_OUT(bit);
	PIN_SWCLK_CLR();
	PIN_DELAY_FAST();
	PIN_SWCLK_SET();
	PIN_DELAY_FAST(); /* APnDP Bit */
	parity += bit;
	bit = request >> 1;
	PIN_SWDIO_OUT(bit);
	PIN_SWCLK_CLR();
	...
	{ /* OK response */
		...
		if (request & DAP_TRANSFER_RnW)
		{
			...
			for (n = 32U; n; n--)
			{
				...
				bit = PIN_SWDIO_IN();
				PIN_SWCLK_SET();
				PIN_DELAY_FAST(); /* Read RDATA[0:31] */
				parity += bit;
				val >>= 1;
				val |= bit << 31;
			}
		...
		else
		{
			...
			for (n = 32U; n; n--)
			{
				...
				PIN_DELAY_FAST();
				PIN_SWCLK_SET();
				PIN_DELAY_FAST(); /* Write WDATA[0:31] */
				parity += val;
				val >>= 1;
			}
			PIN_SWDIO_OUT(parity);
	...
	if ((ack == DAP_TRANSFER_WAIT) || (ack == DAP_TRANSFER_FAULT))
	{
				...
				&& ((request & DAP_TRANSFER_RnW) != 0U))
		{
			for (n = 32U + 1U; n; n--)
			{
				...
				PIN_DELAY_FAST();
				PIN_SWCLK_SET();
				PIN_DELAY_FAST(); /* Dummy Read RDATA[0:31] + Parity */
			}
		}
		/* Turnaround */
		for (n = DAP_Data.swd_conf.turnaround; n; n--)
